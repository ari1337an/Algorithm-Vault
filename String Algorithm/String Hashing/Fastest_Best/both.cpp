...
struct StringHash{
    ...
    uint64_t forward_hash(){ return this->forward_hash(0,this->size-1);}
    uint64_t reverse_hash(){ return this->reverse_hash(0,this->size-1);}
    uint64_t forward_hash(int l, int r){
        int64_t h = pref[r + 1] - modmul(base_pow[r - l + 1], pref[l]);
        return h < 0 ? h + mod : h;
    }
    uint64_t reverse_hash(int l, int r){
