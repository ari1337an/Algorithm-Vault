...
struct SingleHash{
	...
	ull forward_hash(int l,int r){
		ll h;
		if(base == BASE1){
			h = prefix_hash[r+1] - ((ull)basepow1[r-l+1] * prefix_hash[l] % mod) ;
		}else{
			h = prefix_hash[r+1] - ((ull)basepow2[r-l+1] * prefix_hash[l] % mod) ;
		}
		return h < 0 ? h + mod : h;
	}
	...
	ull reverse_hash(int l,int r){
		ll h;
		if(base == BASE1){
			h = (suffix_hash[l+1] -  ((ull)basepow1[r-l+1] * suffix_hash[r+2] % mod) );
		}else{
			h = (suffix_hash[l+1] -  ((ull)basepow2[r-l+1] * suffix_hash[r+2] % mod) );
		}
		return h < 0 ? h+mod : h;
	}
};
...
struct StringHash{
	...
	StringHash(string str, bool singleHash = false, bool onlyPrefix = false): singleHash(singleHash), str(str), size_str(str.size()), onlyPrefix(onlyPrefix) {
		if(singleHash){
	...
	ull forward_hash(){
		if(singleHash) return h1.forward_hash(0,size_str-1);
		else return ((ull)h1.forward_hash(0,size_str-1) << 32) ^ h2.forward_hash(0,size_str-1);
	}

	ull forward_hash(int l,int r){
		if(singleHash) return h1.forward_hash(l,r);
		else return ((ull)h1.forward_hash(l,r) << 32) ^ h2.forward_hash(l,r);
	}

	ull reverse_hash(){
		if(!onlyPrefix) {
			if(singleHash) return h1.reverse_hash(0,size_str-1);
			else return ((ull)h1.reverse_hash(0,size_str-1) << 32) ^ h2.reverse_hash(0,size_str-1);
		}
		else{
	...
	ull reverse_hash(int l,int r){
		if(!onlyPrefix) {
			if(singleHash) return h1.reverse_hash(l,r);
			else return ((ull)h1.reverse_hash(l,r) << 32) ^ h2.reverse_hash(l,r);
		}
		else{
			cerr << "Reverse Hash Was Not Calculated Upon Hashing" close
			return 0;
