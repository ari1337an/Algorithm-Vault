...
void generate_sparse_RMQ(vector<int> &array, int N){
    ...
    for (int j = 1; j <= K; j++) for (int i = 0; i + (1 << j) <= N; i++)
        st[i][j] = min(st[i][j-1], st[i + (1 << (j - 1))][j - 1]); // change the min to max for Range Max Query
}

int RMQ(int L, int R){
    int j = log_val[R - L + 1];
    int minimum = min(st[L][j], st[R - (1 << j) + 1][j]); // change the min to max for Range Max Query
    return minimum;
}
